  ...
  drawPossibleSpreadTicks(){
    ...
    var maxSpread = parseInt(otreeConstants.maxSpread);
        ...
        for(;temp < maxSpread;){
            ...
            //     .attr("x", (spreadGraph.spread_width / 2) - 24)  
            //     .attr("y",  svg_middle_y + y_coordinate - 4)
            ...
            spreadGraph.spread_svg.append("text")
                .attr("text-anchor", "start")
                .attr("x", (spreadGraph.spread_width/2) - 24) 
                .attr("y",  svg_middle_y - y_coordinate + 10)
                .attr("class", "price-grid-line-text")
                .text((temp/10000).toFixed(2));     
 ...
 addOthersLineAnimation(lines, speed=500, width){
  ...
  drawQueue(){
    ...
    for(var price in spreadGraph.queue){
        index = parseInt(spreadGraph.queue[price].indexOf(userPlayerID));
        console.log(index);
        console.log(index != -1);
        if(index != -1){
            console.log("This");
